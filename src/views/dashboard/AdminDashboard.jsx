import React from 'react';
import SectionTitle from '../../components/common/SectionTitle';
import StatusBadge from '../../components/ui/StatusBadge';
import { Table, TableRow, TableCell } from '../../components/ui/Table';
import Button from '../../components/ui/Button';
import { formatCurrency } from '../../utils/formatters';
import { Shield, Users, Building, AlertCircle } from 'lucide-react';

const systemLogs = [
  {
    id: 'USR-801',
    name: 'Sarah Jenkins',
    email: 'sarah.host@rentify.com',
    role: 'Host',
    status: 'Active',
  },
  {
    id: 'USR-802',
    name: 'Alex Morgan',
    email: 'alex.morgan@example.com',
    role: 'Tenant',
    status: 'Active',
  },
];

export const AdminDashboard = () => {
  return (
    <div className="space-y-8">
      <SectionTitle
        badge="Platform Administration"
        title="Admin Dashboard"
        subtitle="Manage registered users, inspect host properties, and review system operations."
      />

      
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Users</p>
            <h3 className="text-2xl font-black text-slate-900 mt-1">1,420 Users</h3>
          </div>
          <div className="p-3 bg-blue-50 text-blue-700 rounded-xl border border-blue-100">
            <Users className="w-6 h-6" />
          </div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total Listings</p>
            <h3 className="text-2xl font-black text-slate-900 mt-1">350 Homes</h3>
          </div>
          <div className="p-3 bg-slate-100 text-slate-800 rounded-xl border border-slate-200">
            <Building className="w-6 h-6" />
          </div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex items-center justify-between">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Platform Commission</p>
            <h3 className="text-2xl font-black text-slate-900 mt-1">{formatCurrency(128400)}</h3>
          </div>
          <div className="p-3 bg-emerald-50 text-emerald-700 rounded-xl border border-emerald-100">
            <Shield className="w-6 h-6" />
          </div>
        </div>
      </div>

      
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-slate-900">User & Security Management</h3>
        <Table headers={['User ID', 'Name & Email', 'Role', 'Status', 'Actions']}>
          {systemLogs.map((u) => (
            <TableRow key={u.id}>
              <TableCell className="font-mono font-bold text-xs text-blue-700">{u.id}</TableCell>
              <TableCell>
                <div>
                  <p className="font-bold text-slate-900 text-xs sm:text-sm">{u.name}</p>
                  <p className="text-xs text-slate-500">{u.email}</p>
                </div>
              </TableCell>
              <TableCell className="font-semibold text-slate-700">{u.role}</TableCell>
              <TableCell>
                <StatusBadge status={u.status} />
              </TableCell>
              <TableCell align="right">
                <Button variant="ghost" size="sm">
                  Manage Role
                </Button>
              </TableCell>
            </TableRow>
          ))}
        </Table>
      </div>
    </div>
  );
};

export default AdminDashboard;
